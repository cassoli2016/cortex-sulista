# -*- coding: utf-8 -*-
"""Bancada do app do agregado: schema descartável e um WhatsApp de mentira.
...
`agregado.ESQUEMA` (vínculo, código, sessão) e `auth.ESQUEMA` (o `audit_log`,
...
from api import agregado, auth, pglocal


@pytest.fixture
def esq(esquema_pg, monkeypatch):
    monkeypatch.setattr(agregado, "ESQUEMA", esquema_pg)
...
def zap(monkeypatch):
    ...
    pobre que o real é como um caminho de erro passa despercebido. Guarda o
    texto para o teste provar que o código SAIU na mensagem — e nada mais o
    guarda em lugar nenhum, porque no banco só existe o SHA-256.
    """
    ...
    monkeypatch.setattr(aent.wa, "enviar", falso)
    return enviadas


def cadastrar(esquema: str, codigo: str, telefone: str, nome: str = "Transportes Fulano",
              ativo: bool = True) -> int:
    ...
    """
    linha = pglocal.um(
        """INSERT INTO agr_vinculos(proprietario_codigo, telefone, nome, ativo)
