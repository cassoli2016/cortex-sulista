...
def _serializacao(base) -> None:
    ...
    def serializa(self, ds, pretty_print=False):
        ...
        # PROPRIO na funcao original, ANTES do `.export()` — e a versao
        # anterior deste remendo mandava os dois para o xsdata, que responde
        # "Type <class 'str'> is not a dataclass". Nao aparecia na consulta de
        # status (que nunca serializa string) e so estourou ao assinar de
...
def _payload_comprimido(base, transmissao) -> None:
    ...
    base64 — que é o que o serviço `cteRecepcaoSinc` espera. Só que o
    `_post` da base chama `_generateds_to_string_etree(raiz)` e o
    `interpretar_mensagem` da transmissão chama `etree.fromstring`, os dois
    sem perguntar se aquilo é XML. O resultado é
    "Start tag expected, '<' not found" com o payload CORRETO na mão.

    Este é o caminho que a NF-e não exercita: lá o envio vai em XML puro.
    """
    alvo = base.DocumentoEletronico
...
def compatibilizar():
    ...
    if not _APLICADO:
        ...
        _APLICADO = True
    _endereco(cte)
    # ORDEM IMPORTA: `_serializacao` tem de embrulhar antes, para que o
    # remendo do payload comprimido fique por FORA e decida primeiro se
    # aquilo é XML. Invertido, o base64 chegaria ao xsdata.
    _serializacao(base)
