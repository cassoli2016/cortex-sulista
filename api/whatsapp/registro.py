...
def contar_destinatarios_hoje(esquema: str | None = None) -> int:
    """Números DISTINTOS que receberam mensagem hoje, com sucesso.
    ...
    r = pglocal.um(
        ...
        " WHERE ok=1 AND ts >= %s", (hoje + " 00:00:00",), esquema=_esq(esquema))
    return int(r["n"] or 0)


def ja_falou_hoje(telefone: str, esquema: str | None = None) -> bool:
    """Se já houve mensagem para ESTE número hoje, mandar outra não gasta
    ...
    r = pglocal.um(
        ...
        " WHERE ok=1 AND telefone=%s AND ts >= %s",
        (telefone, hoje + " 00:00:00"), esquema=_esq(esquema))
    return bool(r["n"])
...
def resumo(esquema: str | None = None) -> dict:
    ...
    return {"total": r["total"] or 0, "ok": r["ok"] or 0,
            ...
            "ultimo": r["ultimo"],
            "hoje": contar_destinatarios_hoje(esquema)}
